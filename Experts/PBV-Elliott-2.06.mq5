...
datetime zzDataTopo[];
int      zzHandle;
input int zzProfundidade = 21; // Profundida das ondas maiores
   ...
   */
   ...
   if(dataFundoAtual > dataTopoAtual) {
      ...
      */
      ...
      takeProfit = NormalizeDouble( precoAtual + (precoTopoAtual - precoFundoAnterior), _Digits) ;
      ...
      */
      ...
      takeProfit = NormalizeDouble( precoAtual - (precoTopoAnterior - precoFundoAtual), _Digits) ;
   ...
   if( isPosicaoAberta ) {
      ...
      if( tipoPosicaoAberta == POSITION_TYPE_BUY) {
         ...
         if( ativoInfo.Last() > precoFundoAtual2 ) {
            ...
            //double valorTakeProfit = NormalizeDouble( ativoInfo.Last() + (precoTopoAtual - precoFundoAnterior), _Digits) ;
            //takeProfit = NormalizeDouble(MathRound(valorTakeProfit / ativoInfo.TickSize()) * ativoInfo.TickSize(), _Digits);

            trade.PositionModify(_Symbol, NormalizeDouble( stopLoss, _Digits), NormalizeDouble(takeProfit, _Digits));
         ...
         if( ativoInfo.Last() < precoTopoAtual2 ) {
            ...
            //double valorTakeProfit = NormalizeDouble( ativoInfo.Last() - (precoTopoAnterior - precoFundoAtual), _Digits) ;
            //takeProfit = NormalizeDouble(MathRound(valorTakeProfit / ativoInfo.TickSize()) * ativoInfo.TickSize(), _Digits);
