...
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Gerson Pereira / Edalício Heinzen /Laura Regina Pereira"
#property description "Robô, baseado nas ondas de Elliott e números de Fibonacci. Possui dois ZigZag's, sendo que o ZigZag com profundidade maior é utilizado para considerar as aberturas de ordens e o ZigZag menor, é utilizado para o Trailling Stop. \n\ngerson@embracore.com.br / edalicio@outlook.com / laureginap@gmail.com"
...
/*
   ...
   +-----------------------------------------------------------------+
   | Disciplina:  Programação para Bolsa de Valores                  |
   | Professor:   Alexandre de Aguiar Amaral                         |
   | Alunos:      Gerson Pereira                                     |
   |              Edalício Heinzen                                   |
   |              Laura Regina Pereira                               |
   ...
   |     Profundidade da Onda Maior....:    12                       |
   |     Profundidade da Onda Menor....:     4                       |
   |     Delta Stop....................: 0.250                       |
   |     Pontos para Stop Loss.........:    13                       |
   |     Timeframe  ...................:    1M                       |
   |     Perído de teste...............: 01/06/2019 ~ 07/12/2019     |
   |                                                                 |
   | -> Mini-Índice Bovespa Futuro:                                  |
   |     Profundidade da Onda Maior....:   23                        |
   |     Profundidade da Onda Menor....:    3                        |
   |     Delta Stop....................: 0.25                        |
   |     Pontos para Stop Loss.........:   20                        |
   |     Timeframe  ...................:   5M                        |
   |     Perído de teste...............: 01/06/2019 ~ 07/12/2019     |
   |                                                                 |
   | -> Ação -> WDO:                                                 |
   |     Profundidade da Onda Maior....:   24 / 25                   |
   |     Profundidade da Onda Menor....:    3 / 5                    |
   |     Delta Stop....................: 0.25 / 0.25                 |
   |     Pontos para Stop Loss.........:   43 / 90                   |
   |     Timeframe  ...................:   5M                        |
   |     Perído de teste...............: 01/06/2019 ~ 07/12/2019     |
   |                                                                 |
   | -> Ação -> VALE3:                                               |
   |     Profundidade da Onda Maior....:   13                        |
   |     Profundidade da Onda Menor....:    7                        |
   |     Delta Stop....................: 0.10                        |
   |     Pontos para Stop Loss.........:  100                        |
   |     Timeframe  ...................:   1M                        |
   |     Perído de teste...............: 01/06/2019 ~ 07/12/2019     |                                                                 |
...
datetime zzDataTopo[];
int      zzHandle;
input int zzProfundidade = 23; // Profundida das ondas maiores
...
int      zzHandle2;
input int zzProfundidade2 = 9 ; // Profundida das ondas menores
...
double stopLoss = 0;
int deltaStop = 0;
input double deltaStopPercentual = 0.10; // Percentual do Delta Stop
   ...
   if(dataFundoAtual > dataTopoAtual) {
      ...
      */
      ...
      if(   precoAtual >= precoVendaRegiao1
            ...
            && volumeAnterior > volumeAtual
            && precoTopoAtual < precoTopoAnterior
            && !( precoFundoAnterior < precoFundoAnteriorC
                  && precoFundoAnterior < precoFundoAtual)) {
...
// Verifica se existe uma posição aberta
   if( isPosicaoAberta ) {
      ...
      if( buscarPosicaoAbertasByTipo(POSITION_TYPE_BUY) == true ) {
         // Se preço atual for MAIOR do que o fundo da onda atual menor, calcula e modifica o trailling stop se necessário
         if( ativoInfo.Last() > precoFundoAtual2 ) {
            for(int i = 0; i < PositionsTotal(); i++) {
               ticket = PositionGetTicket(i);
               if(ticket > 0) {
               
                  newsl = NormalizeDouble(precoFundoAtual2 -  stopLossPontos * ativoInfo.Point(), ativoInfo.Digits());

                  // Se o novo stopLoss é MAIOR do que a posição de stopLoss definida anteriormente, então será
                  // redefinido com base no valor da variável 'newsl'
                  if(newsl > PositionGetDouble(POSITION_SL)) {
                     // Modificar a posição de stopLoss para a posição corrente, com o valor definido em 'newsl'
                     trade.PositionModify(ticket, newsl, PositionGetDouble(POSITION_TP));
                  } // Fim da verificação de condição da variável 'newsl'
               } // Fim da verificação do valor da variável 'ticket'
            } // Fim do laço para buscar posições abertas
         } //Fim da verificação para veirificar se o preço atual é menor do que o preço do topo atual da onda 2
         //Fim do busca posição de COMPRA para processar o trailling stop

      } else if ( buscarPosicaoAbertasByTipo(POSITION_TYPE_SELL) == true) {
         // caso venda
         // preço atual for menor do que o topo da onda atual menor muda o trailling
         if( ativoInfo.Last() < precoTopoAtual2 ) {
            //Laço para buscar posições abertas e processar cada uma delas
            for(int i = 0; i < PositionsTotal(); i++) {
               ticket = PositionGetTicket(i);
               if(ticket > 0) {

                  newsl = NormalizeDouble(precoTopoAtual2 + stopLossPontos * ativoInfo.Point(), ativoInfo.Digits());

                  // Se o novo stopLoss é MENOER do que a posição de stopLoss definida anteriormente, então será
                  // redefinido com base no valor da variável 'newsl'
                  if(newsl < PositionGetDouble(POSITION_SL)) {
                     // Modificar a posição de stopLoss para a posição corrente, com o valor definido em 'newsl'
                     trade.PositionModify(ticket, newsl, PositionGetDouble(POSITION_TP));
      ...
      } // Fim do busca posição de VENDA para processar o trailling stop
   }
} //Fim do OnTick
